...
#define LOCTEXT_NAMESPACE "MyStaticMeshSceneProxy"

static bool GUseShadowIndexBuffer = true;

FMyStaticMeshSceneProxy::FMyStaticMeshSceneProxy(UStaticMeshComponent* Component, bool bForceLODsShareStaticLighting):FStaticMeshSceneProxy(Component,bForceLODsShareStaticLighting),ComponentPtr(Component)
{
...
void FMyStaticMeshSceneProxy::DrawStaticElements(FStaticPrimitiveDrawInterface* PDI)
{
	checkSlow(IsInParallelRenderingThread());
	//----------------------------------------------------------------------
	// 该pass使用FDepthDrawingPolicy策略进行绘制，只绘制depth到Depth-Buffer，
	// 这个有利于减少后面的Base pass中的pixel填充，节省pixel-shader的执行。
	//----------------------------------------------------------------------
	// FVertexFactoryInput,这个数据结构是来自C++端的数据输入。
	// 在不同的Shader头文件中可能会有不同的定义，使用方式上自然也会有不同。
			...
			for (int32 SectionIndex = 0; SectionIndex < LODModel.Sections.Num(); SectionIndex++)
			{
				...
				for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
				{
					...
					if (GetMeshElement(LODIndex, BatchIndex, SectionIndex, PrimitiveDPG, bIsMeshElementSelected, true, MeshBatch))
					{
						...
						const UMyStaticMeshComponent* MyStaticMeshComponent = dynamic_cast<const UMyStaticMeshComponent *>(ComponentPtr);
						if (MaterialInterface == MyStaticMeshComponent->SecondPassMaterial)
						{
							continue;
						}
						if (MyStaticMeshComponent->NeedSecondPass) {
							if (MyStaticMeshComponent->SecondPassMaterial == nullptr) {
								continue;
							}
							MeshBatch.MaterialRenderProxy = MyStaticMeshComponent->SecondPassMaterial->GetRenderProxy();
